...
/*		if ("Search Fingerprints")
		...
		if("Edit Field")
			...
			switch(href_list["field"])
				...
				if("notes")
					if(istype(active2, /datum/data/record))
						var/t1 = sanitize(input("Please summarize notes:", "Secure. records", lhtml_decode(sanitize_local(active2.fields["notes"], SANITIZE_TEMP)), null)  as message, extra = 0, max_length = MAX_RECORD_LENGTH)
						if(!t1 || !authenticated || usr.stat || usr.restrained() || (!interactable() && !issilicon(usr)) || active2 != a2)
							return
