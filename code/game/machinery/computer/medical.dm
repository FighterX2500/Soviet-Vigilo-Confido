		...
		if (src.authenticated)
			...
			if (href_list["field"])
				...
				switch(href_list["field"])
					...
					if("notes")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(input("Please summarize notes:", "Med. records", lhtml_decode(sanitize_local(src.active2.fields["notes"], SANITIZE_TEMP)), null)  as message, extra = 0, max_length = MAX_RECORD_LENGTH)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
